...
const socials = [
  {
    label: "LinkedIn",
    href: "https://www.linkedin.com/company/nurone",
  ...
  {
    label: "Instagram",
    href: "https://www.instagram.com/nurone.io",
  ...
  {
    label: "Facebook",
    href: "https://www.facebook.com/nurone.io",
    ...
    <footer className="border-t border-line bg-void relative overflow-hidden">
      <div className="shell pb-8 pt-16">
        <div className="grid gap-12 border-t border-line/60 pt-12 lg:grid-cols-[1.4fr_1fr_1fr_1.4fr]">
          ...
          <div>
            <Logo />
            <p className="mt-5 max-w-xs text-sm leading-relaxed text-muted/60">
              {siteConfig.tagline}
            </p>
            <div className="mt-6 flex items-center gap-2">
              {socials.map((s) => (
                <a
                  ...
                  aria-label={s.label}
                  data-hover="link"
                  className="inline-flex size-10 items-center justify-center rounded-xl border border-line bg-white/[0.02] text-muted transition-colors hover:border-accent/30 hover:text-accent"
                >
          ...
          {Object.entries(footerNav).map(([heading, links]) => (
            <nav key={heading} aria-label={heading}>
              <h2 className="text-[0.62rem] text-faint/80 uppercase tracking-[0.2em] font-medium">{heading}</h2>
                ...
                {links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      scroll={false}
                      className="link-underline text-sm text-muted/70 transition-colors hover:text-ink"
                    >
                      {link.label}
                    </Link>
                  </li>
          ...
          <div className="max-w-sm">
            <h2 className="text-[0.62rem] text-faint/80 uppercase tracking-[0.2em] font-medium">Stay Updated</h2>
            <p className="mt-3 text-xs leading-relaxed text-muted/50">
              Quarterly engineering insights and AI architecture breakdowns. No spam.
            </p>
            <div className="mt-4">
              <NewsletterForm />
            </div>
        ...
        {/* Legal Footer */}
        <div className="mt-6 flex flex-col items-center justify-between gap-4 border-t border-line/60 pt-8 text-xs text-faint/70 sm:flex-row">
